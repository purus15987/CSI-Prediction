...
from models.LSTM_model import LSTMCSI
from models.transformer_model import TransformerCSI
import torch
...
def train(train_data, valid_data, args, result_file):
    ...
    if(args.model_name == 'transformer'):
        model = TransformerCSI(input_dim=node_cnt, model_dim=196, num_heads=4, num_layers=4, dropout=0.1, max_len=args.window_size, horizon=args.horizon)
    elif(args.model_name == 'lstm'):
        model = LSTMCSI(input_dim=node_cnt, hidden_dim=196, num_layers=4,dropout=0.1, output_dim=node_cnt, horizon=args.horizon)
    else:
...
def test(test_data, args, result_train_file, result_test_file):
    ...
    results = [[args.model_name, args.channel, args.kmph, mape, mae, rmse]]
    ...
    with open(csv_filename, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(['Model', 'Encoded Dim', 'Speed (kmph)', 'MAPE', 'MAE', 'RMSE'])
